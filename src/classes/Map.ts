...
import { NextFrameInfo, SnakeMapData, gameObjectType } from "../@types/MapTypes";
...
export class Tile {
    ...
    constructor(parent: SnakeMap, coordinates: Coordinates, data: Nullable<Entity> = null) {
        this.parent = parent;
...
export class SnakeMap {
    ...
    constructor(serializedMap: string) {
        ...
        if (!startTile) {
            throw new Error(`Invalid start position for snake at ${jsonObj.snake.startPosition}`);
        }

        this.snake = new Snake(startTile);
        this.snake.body.push(...jsonObj.snake.bodyParts.map(part => {
        ...
        const objs = jsonObj.gameObject.map(obj => GameObjectFactory.createGameObject(this, obj));
        objs.forEach(obj => {
            const tile = this.getTile({ x: obj.getLocationTile().x, y: obj.getLocationTile().y });
            if (!tile) {
                throw new Error(`Invalid tile for object at ${obj.getLocationTile()} of type ${obj.constructor.name}`);
            }
            tile.data = obj;
        });
    ...
    getTile(coordinates: Coordinates): Nullable<Tile> {
        return this.tiles.find(tile => tile.x === coordinates.x && tile.y === coordinates.y) || null;
    }
